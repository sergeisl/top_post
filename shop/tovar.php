 ...
 * @property-read String kod_prodact Короткий код(Артикул товара) == kod_prodact код производителя
 ...
 * @property-read integer priceu рекомендуемая розничная цена товара
 ...
 * @property-read integer gr возвращает основную category = gr
// * @property-read integer gr "0-товар, 1-услуга, 2-абонемент, 3-расходка", tTYPE_*
...
class Tovar implements ArrayAccess{
    ...
    const img_name='kod_prodact'; // 'kod_prodact' имя картинки формировать из кода или из id товара
    const tbl_alias='category_alias'; // поддержка таблицы синонимов категорий, если пусто - не использовать
    const tbl_prixod=''; // 'prixod' - поддержка таблицы приходов, если пусто - не использовать
    public $Seo_url=SEO;
    public $message;
    ...
    //static public $ar_gr=array(0=>"Косметика",1=>"Услуги",2=>"Абонементы",3=>"Расходка"); // если не определена, то используется таблица gr
    ...
    function __get($property){// - вызывается при обращении к неопределенному свойству
        ...
        elseif(empty($this->tovar['id'])){
            ...
            if(!isset($this->tovar['category'])){
                ...
                while($row=DB::fetch_assoc($query))$this->tovar['category'][$row['category']]=1; // в этом же формате возвращает $_POST
            }
    ...
    11 	Тело 	тело*/
            ...
            return str_replace("'","`",$this->tovar['name'].
                (!empty($this->tovar['kol'])&&$this->tovar['kol']>1?" ".($this->tovar['kol']==intval($this->tovar['kol'])?intval($this->tovar['kol']):$this->tovar['kol']).$this->tovar['ed']:'').' - '.$this->tovar['price']."руб.");
            ...
            while(($row=DB::fetch_assoc($query))){
                ...
                $out.=",<br> <a".(User::is_admin()?'':" style='white-space:nowrap'")." href='".$shop['url']."' onclick='return !window.open(this.href)'>".$shop['name'].
                    (User::is_admin()?" - ".$row['ost']."шт.".($row['price']==$this->price?'':", ".$row['price']."руб."):"")."</a>";
            }
            ...
            return ($this->tovar['kol']==intval($this->tovar['kol'])?intval($this->tovar['kol']):$this->tovar['kol']).($this->tovar['ed']?$this->tovar['ed']:($this->tovar['gr']?'':'мл'));
        }elseif($property=='child'){
            return $this->tovar['child']=($this->tovar['tovar'] ? new Tovar($this->tovar['tovar']) : null );

        }elseif($property=='price_old'){
            return round($this->tovar['priceu'],0,PHP_ROUND_HALF_UP); // округляю до целого рубля
            ...
            /*        }elseif($property=='img'){
                        ...
                        if($fil && is_file($_SERVER['DOCUMENT_ROOT'].$fil)){
                            ...
                            return "<div class='box left small c' style='height:40px;width:50px' onclick=\"return ajaxLoad('','api.php?form_img=".$this->tovar[self::img_name]."');\">Нет<br>картинки</div>";
                        }*/
        }elseif($property=='row'){
            $row=$this->tovar;
    ...
    */
    ...
    function __set($property,$value){// - вызывается, когда неопределенному свойству присваивается значение
        ...
        if(isset($GLOBALS['tovar_cash['.$this->id.']'])){  // сбрасываю хеш
    ...
    public function __construct($tovar = null){
        ...
        if(is_array($tovar)){
            $this->tovar=$tovar;
            unset($this->tovar['category']); // если я передал лишние поля - удалить их
        }elseif($tovar>0){
            $this->tovar=DB::Select("tovar",intval($tovar)); //$this->tovar=$this->GetTovar($tovar,true);
        }

        if($this->tovar==null){error("Ошибка в коде товара ".var_export($tovar,!0)); return null;}
    ...
    static function GetTovar($tovar,$notChild=false){
        ...
        if($tovar0['tovar']&&!$notChild){
            ...
            }else{  Out::error("Ошибка в базе товаров: нет товара по ссылке ".$tovar0['tovar']." !");
                $tovar=$tovar0;
            }
        }else $tovar=$tovar0;
        ...
        $tovar['show_name']=str_replace("'","`",$tovar['name'].($tovar['kol']>1?" ".($tovar['kol']==intval($tovar['kol'])?intval($tovar['kol']):$tovar['kol']).$tovar['ed']:'').' - '.$tovar['price']."руб.");
    ...
    static function Del($id){
        ...
        $id=intval($id);
        /*        $query=DB::sql("SELECT * FROM `".self::db_prefix."tovar` WHERE tovar='".$id."' LIMIT 1"); // исходный для составного
        ...
        // удаляю записи в поставщиках
        DB::Delete('supplier_link','tovar="'.$id.'"');

        // удаляю несовместимости
    ...
    /** объединяет два товара в $tov
     * @param integer|array $old старый товар, удаляется
     * @param integer|array $tov новый товар, остается
     * @return bool
     */
    static function Union($old,$tov){
        ...
        DB::Delete("supplier_link","tovar='".$id_old."'"); // удаляю проигнорированные
        ...
        self::DelImg($tov_old);
        DB::log('tovar', $id_tov, 'Объединение с '.$id_old);
    ...
    static function Cennik($data){
        ...
        "<div style='text-align:center;margin:0 auto'>Цена: <b style='font-size:22px'>".$data['price']."</b>".(strlen($data['price'])<3?" ":"")."руб.</div>".
        "<div style='font-size:11px;width:100%;text-align:right;vertical-align:sub;'>____________ ИП Колесников Д.Г.</div>".
        "<div style='font-size:12px;float:right;width:14mm;text-align:right'>".($data['kol']==0?'&nbsp;':($data['kol']==intval($data['kol'])?intval($data['kol']):$data['kol']).($data['ed']?$data['ed']:"мл"))."</div>".
        "<div style='font-size:12px;float:left;width:14mm;text-align:left'>".$data['kod_prodact'].($data['kod_prodact']&&$data['ean']?"/":"").$data['ean']."</div>".
        "<div style='margin:0 auto;font-size:12px;width:12mm;text-align:center'>".date("d.m.y",strtotime("+1 day"))."</div>".
        "</div>";
    ...
    /** по наименованию возвращает id группы, учитывает ошибки написания, если не находит и $add - добавляет
     * @param $name
    ...
    static function grList($root=-1, $options=[]){// $format='',$act=0, $add=''){
        ...
        if(empty($options['format'])){
            ...
            } // todo добавить подсчет из category_link
            return $ar[$root];
...
";
            ?>
            <a href="/price/">Все товары, услуги и цены</a>
...
*/
    ...
    static function PrintTovar($row, $options=null){ // , $full = false, $div = false, $noKateg = false ['full'=>true, 'div'=>true, 'noKateg'=>true]
        ...
        if(!$full)print "<a href='/price/tovar".$tovar->id."' target=_blank><img id='img".$tovar->id."' src='".$img1."' /></a>";	// по картинке перехожу в товар
        ...
        if($full){
            ...
            if($desc){
                $desc = trim(preg_replace('#(?<!\])\bhttp://[^\s\[<]+#i',
                    " <noindex><a href=\"/price/?url=$0\" target=_blank><u>Посмотреть на сайте производителя</u></a></noindex> ",
...
END;
            ...
            <div class="layer">
                ...
                <div class='tdes'>
                    <a href='/api.php?pt=<?=$tov['id']?>' onclick="return ajaxLoad('',this.href,'..');" title="Поставщики">Поставщики:</a>
                    <b><?
    ...
    static function ShowSupplier($id){  // нажали кнопку [Поставщики]
        ...
        if(DB::num_rows($result)>0){
            while ($row=DB::fetch_assoc($result)) {
                ...
                echo "<div class='supplier".($i++%2?"2":'')."'>Поставщик: <a href='/price/?pr=".$row['supplier']."'><b>".DB::GetName('supplier',$row['supplier'])."</b></a><br>".toHtml($row['name'])."<br>
...
// не объединять при расхождении цены более 50%
     ...
     * @param $tov ['brand'] если бренд задан наименованием или массивом, переведу в код
     * @return bool
     */
    static function SaveTovar(&$tov){
        ...
        if($tov['id']>0){
            ...
            if($tov['category'])foreach($tov['category'] as $key => $value)if(empty($row['category'][$key])&&$value!='0'){
                ...
                $f_ok=$f_ok||(DB::affected_rows()>0);
                //if(DB::affected_rows()<1)message("Неверная категория: ".print_r($tov['category'],true)."<br>".print_r($row['category'],true));
            }
        ...
        if(strlen($tov['kod_prodact'])>3){ // TODO перетащить алгоритм в add.php !!!
            ...
            $rf=' and brand="'.$tov['brand'].'" and gr='.$tov['gr'].' and (lower(kod_prodact)="'.addslashes(mb_strtolower($kod_prodact)).'" or LOWER(name) REGEXP "[^A-Za-zА-Яа-я0-9\+]'.DB::escape(preg_quote(mb_strtolower($kod_prodact))).'[^A-Za-zА-Яа-я0-9\+/]")';
            $result=DB::sql('SELECT * from '.db_prefix.'tovar WHERE id<>'.$tov['id'].$rf);
            while ($row=DB::fetch_assoc($result)) if(!in_array($row['id'],$aid)){
                //if(abs($row['price']-$tov['price'])/$tov['price']>0.5){echo '<br>Не объединил <a href="/price/tovar'.$id.'">'.$id.'</a> и <a href="/price/tovar'.$row['id'].'">'.$row['id'].'</a> из-за большой разницы в цене'; continue;}
                Tovar::AskObedin($tov,$row);
                $aid[]=$row['id'];
            }
        }
...
// если цена товара до 100рублей, то делаю наценку не меньше 100%
    ...
    static function cPrice(&$tov, $supplier=0, $Show=false){
        ...
        $price0_r=floatval($price0) * self::Kurs($tov['valuta0']); // закупочная цена в рублях
